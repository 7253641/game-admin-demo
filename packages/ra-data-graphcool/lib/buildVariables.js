    ...
    var filter = Object.keys(params.filter).reduce(function (acc, key) {
        ...
        if (parts.length > 1) {
            ...
            if (resourceField.type.name === 'Int') {
                return __assign({}, acc, (_d = {}, _d[key] = parseInt(params.filter[key], 10), _d));
            }
            if (resourceField.type.name === 'Float') {
                return __assign({}, acc, (_e = {}, _e[key] = parseFloat(params.filter[key], 10), _e));
            }
        }
        return __assign({}, acc, (_f = {}, _f[key] = params.filter[key], _f));
    }, {});
    return {
